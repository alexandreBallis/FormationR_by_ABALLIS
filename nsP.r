nsP <- function(gere,monnoserie,listnoserie) #Vers une prédicition d'une n° de série à partir d'une faute d'orthographe ou de saisie. 
...
#modele.kernlab <- ksvm(is_modele~.,data=df.training,type="C-bsvc",C=1/2,kernel="rbfdot",prob.model=TRUE)		
#------------------------------------------------------------------------------------------------------------------
...
a <- modele.svm ; x <- parse3(monnoserie,pattern)
...
print(lprop)
#------------------PREDICTIBILITE DU MODELE de la liste en parametres ----------------------------------------------------
#--package (plyr) et la fonction ddply permet d'appliquer à chaque colonne d'un tibble une fonction définie 
#exemple : df <- tibble(x=1:10) (package dplyr)
# mutate(df,score=x^2+y^2)
library(plyr)
n <- length(listnoserie)
lprop <- sapply(1:n,function(i) {predict(modele.kernlab,parse3(listnoserie[i],pattern),type="probabilities")})
lprop <- t(lprop)
colnames(lprop) <- modelsEtudies
...
#Traitement spécifique de la liste en paramètre#
print("scoring de classification établi par le modèle")
df.result 	<- data.frame("no.serie"=listnoserie,lprop)
print(df.result)
return(list(	df.result,
				head(data.training),
				df.test,
				df.CorrAnalyse,
				ksvmTab,modele.kernlab,reg,viewPCA))
